...
@section('content')
    ...
    <div class="card adm">
        ...
        <div class="card-body">
            <form name="personForm" id="personForm" action="/auth/adiministracao/update/{{ $user->id }}" method="POST"
                ...
                <div class="row">
                    ...
                    <div class="col">
                        ...
                        <select class="form-select" aria-label="Default select example" name="sexo" id="sexo">
                            ...
                            <option value="ND" @if ($user->sexo == 'ND') {{ 'selected' }} @endif>Prefiro Não
                                Declarar</option>
                            <option value="M" @if ($user->sexo == 'M') {{ 'selected' }} @endif>Masculino
                            </option>
                            <option value="F" @if ($user->sexo == 'F') {{ 'selected' }} @endif>Feminino
                            </option>
                ...
                <div class="row">
                    ...
                    <div class="col">
                        ...
                        <select class="form-select block mt-1 w-full" aria-label="Default select example" name="estado_id"
                            id="estado_id">
                            <option value="{{ $user->cidade->estado->id }}">{{ $user->cidade->estado->nome }}</option>
                            @foreach ($estados as $estado)
                                <option value="{{ $estado->id }}"
                                    @if ($user->cidade == '{{ $estado->id }}') {{ 'selected' }} @endif> {{ $estado->nome }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <input type="tel" class="form-control" id="telefone" name="telefone"
                            value="{{ $user->telefone }}" onkeypress="$(this).mask('(00) 0000-00009')" required />
                    </div>
                    <div class="col">
